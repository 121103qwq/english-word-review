...
8.0.0 的共享核心将 7.4.5 行为登记为 `weighted-random-v1`。时间戳和复习间隔只随事件记录，不参与抽题。
...
压缩检查点保存设备序号向量和祖先检查点。只有配置的两个镜像都成功读取时才压缩；单镜像或读取失败时保留活动事件。SHA、ETag 或 `If-None-Match` 冲突会重新读取、合并并只重试一次。
